...
export function PinResetPanel({
  ...
  const inputCls =
    "border-border bg-surface-2 text-foreground placeholder:text-subtle focus:border-primary-400 mt-1.5 w-full rounded-[13px] border py-3 text-center font-black tabular-nums outline-none";
  ...
  return (
    <div className="bg-surface w-full max-w-[320px] rounded-[26px] p-6 text-center shadow-[0_20px_50px_-16px_rgba(0,0,0,.4)]">
      ...
      {done ? (
        <p className="text-success-700 mt-3 text-[13.5px] font-bold">
          {t("pinResetDone")}
        </p>
      ) : step === "send" ? (
        <>
        ...
        <>
          {sentTo && (
            <p className="text-muted mt-1 text-[12.5px] font-medium">
              {t("pinResetSent", { email: sentTo })}
            </p>
          )}
          <label className="text-muted mt-4 block text-start text-[11.5px] font-extrabold tracking-wide uppercase">
            {t("pinResetCodeLabel")}
          </label>
          <input
            inputMode="numeric"
          ...
          <button
            ...
            disabled={busy}
            onClick={send}
            className="text-primary-600 mt-3 text-[12.5px] font-bold disabled:opacity-40"
          >
            {t("pinResetSend")}
          </button>
        </>
