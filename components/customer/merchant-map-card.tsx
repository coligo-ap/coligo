...
function pinElement(): HTMLDivElement {
  ...
  el.innerHTML =
    '<div style="width:30px;height:30px;border-radius:50% 50% 50% 4px;transform:rotate(45deg);background:#6C2BD9;display:flex;align-items:center;justify-content:center;border:2.5px solid #fff;box-shadow:0 6px 14px -3px rgba(0,0,0,.45)"><svg width="14" height="14" viewBox="0 0 24 24" style="transform:rotate(-45deg)" fill="none" stroke="#fff" stroke-width="2.4" stroke-linecap="round" stroke-linejoin="round"><path d="M3 9.5 12 4l9 5.5"/><path d="M5 10v9h14v-9"/></svg></div>';
  return el;
}
    ...
    <div className="border-border overflow-hidden rounded-[14px] border">
      ...
      <div className="flex items-center gap-3 px-3.5 py-3">
        <span className="min-w-0 flex-1">
          {address && (
            <span className="text-foreground flex items-start gap-1.5 text-[13px] font-semibold">
        ...
        <a
          ...
          target="_blank"
          rel="noopener"
          className="bg-primary-600 hover:bg-primary-700 inline-flex shrink-0 items-center gap-1.5 rounded-full px-3.5 py-2 text-[12.5px] font-extrabold text-white"
        >
        ...
        <Portal>
          <div className="fixed inset-0 z-[110] flex flex-col bg-white">
            ...
            <div className="border-border flex items-center gap-3 border-t bg-white px-4 py-3 pb-[calc(env(safe-area-inset-bottom)+0.75rem)]">
              <span className="min-w-0 flex-1">
                <span className="text-foreground block truncate text-[14px] font-bold">
                  {name}
                </span>
                {address && (
                  <span className="text-muted block truncate text-[12px] font-medium">
                    {address}
                  </span>
                )}
              </span>
              <a
                ...
                target="_blank"
                rel="noopener"
                className="bg-primary-600 hover:bg-primary-700 inline-flex shrink-0 items-center gap-1.5 rounded-full px-4 py-2.5 text-[13px] font-extrabold text-white"
              >
