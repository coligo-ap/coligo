    ...
    <div className="bg-background text-foreground min-h-dvh">
      ...
      <header className="border-border bg-surface/95 sticky top-0 z-10 border-b backdrop-blur">
        <div className="mx-auto flex max-w-6xl flex-wrap items-center gap-3 px-6 py-3">
          ...
          {/* Recherche : filtre les démos sur leur nom, leur fichier source et
              leurs synonymes. Purement client — aucun aller-retour serveur. */}
          <div className="relative order-last w-full sm:order-none sm:w-72">
            ...
            <Input
              ...
              placeholder="Rechercher un composant, un token…"
      ...
      <DsSearchProvider query={query}>
        <div className="mx-auto flex max-w-6xl gap-8 px-6">
          {/* Sommaire persistant, façon bibliothèque : on garde la carte des
              familles sous les yeux pendant qu'on fait défiler. */}
          <nav
            aria-label="Familles de composants"
            className="sticky top-24 hidden h-fit w-48 shrink-0 lg:block"
          >
            <p className="text-nano-lg text-subtle mb-2 font-bold tracking-wide uppercase">
              Familles
            </p>
            <ul className="space-y-0.5">
              {SECTIONS.map(([id, label]) => (
                <li key={id}>
                  <a
                    href={`#${id}`}
                    className="rounded-control text-label-lg text-muted hover:bg-surface-2 hover:text-foreground block px-2.5 py-1.5 font-semibold"
                  >
                    {label}
                  </a>
                </li>
          ...
          <main
            dir={rtl ? "rtl" : "ltr"}
            data-space="client"
            className="min-w-0 flex-1 space-y-8 pb-24"
          >
            <DsSection
              id="palette"
              title="Palette"
              ...
              <DsDemo title="Feuille / modale" source="components/ui/sheet.tsx">
                ...
                >
                  <p className="text-body-sm text-muted">
                    Contenu de la feuille. Elle se ferme avec Échap, par le
                    voile, ou par la croix — et respecte la zone sûre du bas sur
                    mobile.
                  </p>
                </Sheet>
              </DsDemo>
