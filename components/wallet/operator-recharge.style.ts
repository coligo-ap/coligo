 ...
 *
 * Les VARIABLES du thème `.cgw` (clair et sombre) sont déclarées dans
 * app/design-tokens.css (couche 4) — ce fichier ne fait plus que les
 * consommer. Avant, il redéclarait sa propre palette en dur : un fork complet
 * du design system, dont les valeurs avaient dérivé de celles de l'app.
 */
...
/* HERO */
.cgw .hero{position:relative;border-radius:26px;padding:20px;color:var(--color-on-brand);overflow:hidden;background:var(--hero-gradient);box-shadow:0 20px 40px -18px var(--violet-glow)}
...
.cgw .hero .wic svg{width:21px;height:21px;stroke:var(--color-on-brand)}
...
.cgw .hero .ctx svg{width:15px;height:15px;stroke:var(--color-on-brand);flex:none;margin-top:1px}
...
.cgw .m.on .mi{background:var(--color-plain-white)}
...
.cgw .btn{width:100%;height:54px;border:0;border-radius:16px;background:linear-gradient(135deg,var(--violet-l),var(--violet));color:var(--color-on-brand);font-family:var(--sora);font-weight:700;font-size:15px;display:flex;align-items:center;justify-content:center;gap:10px;cursor:pointer;box-shadow:0 14px 28px -12px var(--violet-glow);margin-top:14px}
.cgw .btn:disabled{opacity:.5;cursor:default}
.cgw .btn svg{width:19px;height:19px;stroke:var(--color-on-brand)}
...
.cgw .drop.done .di{background:var(--color-plain-white)}.cgw .drop.done .di svg{stroke:var(--go)}
...
.cgw .vl button.on{background:var(--violet);color:var(--color-on-brand)}
.cgw .citybox{display:flex;gap:8px;margin-bottom:11px}
.cgw .citybtn{border:0;border-radius:14px;background:var(--violet);color:var(--color-on-brand);font-family:var(--sora);font-weight:700;font-size:13px;padding:0 18px;cursor:pointer;display:flex;align-items:center;justify-content:center;min-width:64px}.cgw .citybtn:disabled{opacity:.5}.cgw .citybtn svg{width:18px;height:18px;stroke:var(--color-on-brand)}
...
.cgw .agent .am b>i.agbadge{font-style:normal;font-size:10px;font-weight:800;color:var(--color-on-brand);background:var(--go);width:14px;height:14px;border-radius:50%;display:inline-flex;align-items:center;justify-content:center;flex:none}
...
.cgw .agent .agmeta>i.agopen{color:var(--go)}
...
.cgw .agfilters button.on{background:var(--violet);border-color:var(--violet);color:var(--color-on-brand)}
.cgw .agcount{margin-inline-start:auto;font-size:11px;font-weight:700;color:var(--muted)}
.cgw .miniroute{width:100%;height:42px;border:0;border-radius:12px;background:var(--violet);color:var(--color-on-brand);font-weight:700;font-size:13px;display:flex;align-items:center;justify-content:center;gap:8px;cursor:pointer;margin-top:7px;font-family:inherit}.cgw .miniroute svg{width:15px;height:15px;stroke:var(--color-on-brand)}
...
.cgw .cgw-support{display:flex;align-items:center;justify-content:center;gap:7px;width:100%;margin-top:18px;padding:11px 12px;border:1px solid var(--line);border-radius:13px;background:transparent;color:var(--muted);font-weight:600;font-size:12.5px;cursor:pointer;font-family:inherit}
...
.cgw-sheet-card{background:var(--surface);width:100%;max-width:380px;border-radius:18px;padding:16px;color:var(--ink)}
.cgw-sheet-t{font-family:var(--font-display),'Sora',sans-serif;font-weight:800;font-size:16px;margin:0 0 4px}
.cgw-sheet-s{color:var(--muted);font-size:12px;margin:0 0 12px}
.cgw-sheet-row{display:flex;align-items:center;gap:12px;width:100%;border:1px solid var(--line);border-radius:12px;padding:11px 12px;background:transparent;font-weight:700;font-size:14px;cursor:pointer;margin-bottom:8px;text-align:start;color:inherit;font-family:inherit}
.cgw-sheet-cancel{width:100%;text-align:center;border:0;background:transparent;color:var(--muted);font-weight:600;font-size:14px;cursor:pointer;margin-top:4px;font-family:inherit}
