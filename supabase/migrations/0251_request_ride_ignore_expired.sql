-- =============================================================================
-- 0251 — request_ride : ne plus bloquer sur une recherche EXPIRÉE
-- =============================================================================
-- Bug : l'anti-spam « une seule course active » comptait AUSSI les demandes
-- 'searching' EXPIRÉES (pas de réponse). Conséquence : le client recevait
-- « Vous avez déjà une course en cours » alors que sa recherche était morte —
-- et le bouton « Reprendre » ne trouvait rien (getDriveActiveRide annule d'abord
-- les recherches expirées → my_active_ride NULL → aucune redirection).
--
-- Correctif : avant le contrôle, on PURGE les recherches expirées (deadline,
-- mig 0250) ; et le contrôle EXCLUT explicitement les 'searching' expirées.
-- → plus de faux conflit ; un vrai conflit (course réellement active) reste
-- bloqué ET récupérable (la course existe donc la redirection marche).
-- =============================================================================

CREATE OR REPLACE FUNCTION public.request_ride(p_pickup_lat double precision, p_pickup_lng double precision, p_pickup_text text, p_dest_lat double precision, p_dest_lng double precision, p_dest_text text, p_distance_km numeric, p_proposed_price integer, p_payment_method text DEFAULT 'cash'::text, p_gamme text DEFAULT 'classic'::text, p_boost_da integer DEFAULT 0, p_female_only boolean DEFAULT false, p_proxy_name text DEFAULT NULL::text, p_proxy_phone text DEFAULT NULL::text, p_operation_id text DEFAULT NULL::text, p_pickup_wilaya text DEFAULT NULL::text, p_pickup_commune text DEFAULT NULL::text, p_dest_wilaya text DEFAULT NULL::text, p_dest_commune text DEFAULT NULL::text)
 RETURNS uuid
 LANGUAGE plpgsql
 SECURITY DEFINER
...
BEGIN
  ...
  -- Purge des recherches expirées sans réponse (deadline, mig 0250) AVANT le
  -- contrôle anti-spam : une recherche morte ne doit jamais bloquer une demande.
  ...
  -- Anti-spam : une seule course active à la fois par client. Une 'searching'
  -- EXPIRÉE ne compte PAS (elle est morte / vient d'être annulée ci-dessus).
  IF EXISTS (SELECT 1 FROM public.rides WHERE customer_id = v_customer
             ...
             AND (status <> 'searching' OR expires_at IS NULL OR expires_at > now())) THEN
