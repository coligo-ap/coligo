-- ============================================================================
-- 0371 — IDV : REPLI MANUEL après un refus automatique.
--
-- Un refus automatique ne doit pas être une impasse. Un document abîmé, une
-- photo prise de nuit, un visage qui a changé : la machine se trompe, et
-- l'humain doit pouvoir reprendre la main. L'utilisateur refusé peut désormais
-- demander une VÉRIFICATION MANUELLE — ses pièces, et les captures de la
-- tentative automatique, sont examinées par l'équipe Coligo.
--
-- `manual_fallback` marque ce dossier : il a quitté la voie automatique.
--   • Le serveur autorise alors l'envoi du dossier de l'espace en voie manuelle,
--     même quand la vérification automatique est OBLIGATOIRE — sinon un livreur
--     refusé à tort resterait bloqué pour toujours, sans recours.
--   • L'invariant « approuvé sans face_match » ne s'applique plus à lui : ce
--     dossier est approuvé par un HUMAIN sur pièces, il n'a par construction
--     aucun score de comparaison des visages. L'invariant reste ENTIER pour
--     toutes les décisions automatiques (c'est là qu'il protège).
-- ============================================================================

ALTER TABLE public.idv_verifications
  ADD COLUMN IF NOT EXISTS manual_fallback boolean NOT NULL DEFAULT false;

COMMENT ON COLUMN public.idv_verifications.manual_fallback IS
  'Dossier basculé en vérification MANUELLE après un refus automatique : décidé par l''équipe Coligo sur pièces, sans score de comparaison des visages.';

CREATE INDEX IF NOT EXISTS idx_idv_verifications_manual_fallback
  ON public.idv_verifications (manual_fallback)
  WHERE manual_fallback;

-- ── integrity_violations() : même corps qu'en 0368, un seul invariant amendé ─
