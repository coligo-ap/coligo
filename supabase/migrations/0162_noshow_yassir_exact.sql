-- =============================================================================
-- 0162 — No-show : alignement EXACT sur Yassir Food (enquête terrain)
-- =============================================================================
-- Règle confirmée sur le terrain : en cas de client injoignable sur une
-- livraison ESPÈCES, Yassir rembourse au livreur l'argent qu'il a avancé au
-- commerçant — mais NE PAIE PAS la course ni le déplacement.
--
-- Changements vs 0160 :
--   • ESPÈCES : SUPPRESSION de l'indemnité de course (driver_payout driver_net)
--     versée au livreur depuis 0114. Il ne reste que la réclamation d'avance
--     (remboursée après validation support — inchangé).
--   • ONLINE PAYÉ : la course reste payée au livreur (driver_net) — ce n'est
--     pas une indemnité : le client a déjà payé la livraison, l'argent existe.
--   • Pénalité client (prélèvement de D sur cashback/Coligo Pay + drapeau
--     noshow_pending) : restreinte aux commandes ESPÈCES. Un client online a
--     déjà tout payé sans être remboursé — le re-prélever était une double
--     pénalité. noshow_count++ reste compté dans tous les cas (anti-abus).
--   • Commerçant : inchangé (payé à l'avance au pickup en espèces ; crédité
--     comme une complétion si online payé — 0160).
-- =============================================================================
...
BEGIN
  ...
  -- 2) Course payée UNIQUEMENT si commande ONLINE PAYÉE (l'argent du client
  --    existe : livraison facturée par carte). ESPÈCES : règle Yassir — pas de
  --    paiement de course/déplacement, seule l'avance est réclamable (2bis).
  IF v_driver_net > 0 AND NOT v_is_cash AND v_order.payment_status = 'paid' THEN
  ...
  IF v_is_cash AND v_order.delivery_mode = 'express' THEN
  ...
  -- 3) Pénalité client — ESPÈCES uniquement (un client online a déjà tout payé
  --    sans remboursement). On récupère la course (D) sur cashback PUIS Coligo
  --    Pay, si dispo ; le reste non couvert pose le drapeau noshow_pending.
  --    noshow_count++ dans tous les cas (suivi anti-abus).
  IF v_order.customer_id IS NOT NULL THEN
    ...
    UPDATE public.customers
       SET noshow_count   = noshow_count + 1,
           -- pénalité prochaine commande seulement si espèces non recouvrées
